...
class Data:
    ...
    def __init__(self, 
        ...
        # repeat masks for views?
        repeat_seg = len(rgb_img_names) // len(list(pathlib.Path(self.seg_path).glob('./*')))

        if repeat_seg > 1:
            _seg_img_names = seg_img_names
            seg_img_names = []

            for name in _seg_img_names:
                for i in range(repeat_seg):
                    seg_img_names.append(name)
