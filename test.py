...
from model.rho_nlr import RhoNeuralLumigraph
from lib.sphere_tracer import SphereTracer
from lib.rho_data import RhoData

from conf import config

def get_args():
    parser = argparse.ArgumentParser(description='Evaluate a rho-NLR model.')
    
    parser.add_argument('--dataset_path', type=str, default='./data/rhoblox_dataset/bunny', help='Path to dataset root folder.')
    parser.add_argument('--model', type=str, default='h5/bunny', help='Model folder.')
    parser.add_argument('--img_size', type=int, default=551, help='Maximum image dimension.')
...
def test(opt, nlr=None):
    ...
    if nlr is None:
        tf.print('Creating model.nlr.RhoNeuralLumigraph object from config as one was not provided (nlr=None)')
        
        ''' Data '''
        
        nlr_data = RhoData(opt.dataset_path, img_size=opt.img_size)
        nlr_data.compute_rays(scene_radius_scale=opt.data['scene_radius_scale'])
        nlr_data.load_light_rays()
        nlr_dataset = nlr_data.compute_dataset(v_img=opt.data['v_img'])

        ''' Objects '''
        
        nlr = RhoNeuralLumigraph(**opt.nlr)
